...
namespace Barliesque.InspectorTools.Editor
{
	abstract public class PropertyDrawerHelper : PropertyDrawer
	{
		...
		{
			var obj = fieldInfo.GetValue(_property.serializedObject.targetObject);
			if (obj == null) { return null; }
 
			T actualObject = null;
			if (obj.GetType().IsArray)
			{
				var index = Convert.ToInt32(new string(_property.propertyPath.Where(char.IsDigit).ToArray()));
